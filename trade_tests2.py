        ...
        if len(trades)>=12:
            ...
            try:
                try:
                    ...
                    candles = await account.get_historical_candles(symbol=symbol, timeframe=timeframe, start_time=None, limit=10)
                ...
                if not df.empty:
                    ...
                    df = df.drop(columns=['symbol', 'timeframe', 'brokerTime','volume','spread'])
                    scaler_X = MinMaxScaler(feature_range=(0, 0.1))
                    def prepare_df_for_prediction(df, column):
                        # Convert 'time' to datetime
                        df['time'] = pd.to_datetime(df['time'])
                        df['time2'] = df['time'].astype(int) // 10 * 9  # Numerical time feature
                        columns = ['high', 'low', 'close', 'open', 'tickVolume']
                        
                        for col in columns:
                            # Handle outliers by replacing them with surrounding values
                            z_scores = np.abs(stats.zscore(df[col]))
                            threshold = np.percentile(z_scores, 90)
                            outliers = df.index[z_scores > threshold]
                            
                            for outlier in outliers:
                                start = max(0, outlier - 3)
                                end = min(len(df), outlier + 4)
                                surrounding_values = pd.concat([df[col][start:outlier], df[col][outlier+1:end]])
                                df.at[outlier, col] = surrounding_values.mean()

                        # Set 'time' as index
                        df.set_index('time', inplace=True)
                        
                        # Shift the target column for prediction purposes
                        df['target'] = df[column].shift(-1)
                        df = df.dropna()

                        # Select the last row as the feature
                        last_row = df.iloc[-1:].drop(columns=['target'])
                        ...
                        X_last_scaled = X_last#scaler_X.fit_transform(X_last)

                        # Reshape for prediction input (CNN expects 3D shape: (samples, time_steps, features))
                        X_last_scaled = X_last_scaled.reshape((X_last_scaled.shape[0], X_last_scaled.shape[1], 1))

                        return X_last_scaled

                    # Example usage
                    predictions_features = prepare_df_for_prediction(df, 'close')



                    model_close,_= joblib.load(f'Regressors/Double_Model/{symbol}close{timeframe}_pipeline.joblib')
                    model_high,_= joblib.load(f'Regressors/Double_Model/{symbol}high{timeframe}_pipeline.joblib')
                    model_low,_= joblib.load(f'Regressors/Double_Model/{symbol}low{timeframe}_pipeline.joblib')

                    feature_extractor_close=joblib.load(f'Regressors/Double_Model/feature_extractor/{symbol}close{timeframe}_feature_extractor.joblib')
                    feature_extractor_low=joblib.load(f'Regressors/Double_Model/feature_extractor/{symbol}low{timeframe}_feature_extractor.joblib')
                    feature_extractor_high=joblib.load(f'Regressors/Double_Model/feature_extractor/{symbol}high{timeframe}_feature_extractor.joblib')

                    #predictions_features_extracted= feature_extractor_close.predict(scaler_X.inverse_transform(predictions_features.reshape(-1,1))).round(decimal_places(df['close'].iloc[-2]))
                    predictions_features_extracted= feature_extractor_close.predict(predictions_features).round(decimal_places(df['close'].iloc[-2]))
                    predictions_features_new= np.concatenate((predictions_features.reshape(predictions_features.shape[0], -1), predictions_features_extracted), axis=1)
                    

                    next_close=model_close.predict(predictions_features_new)[0].round(decimal_places(df['close'].iloc[-2]))
                    next_low=model_low.predict(predictions_features_new)[0].round(decimal_places(df['close'].iloc[-2]))
                    next_high=model_high.predict(predictions_features_new)[0].round(decimal_places(df['close'].iloc[-2]))
